...
from models.model import DrlDbscan
...
parser.add_argument('--data_path',
                    default='data/Shape-Pathbased.txt',
                    type=str,
...
if __name__ == '__main__':
    ...
    print('\n+-------------------------------------------------------+\n'
          '* Training and testing DRL-DBSCAN *\n'
          '* Implement via Huawei MindSpore *\n'
          '* Paper: Automating DBSCAN via Reinforcement Learning *\n'
          '* Source: https://anonymous.4open.science/r/DRL-DBSCAN *\n'
          '\n+-------------------------------------------------------+\n')
    ...
    if "Shape" in args.data_path:
        idx_reward, features, labels = load_data_shape(args.data_path,
                                                       args.train_size)
        idx_reward, features, labels = [idx_reward], [features], [labels]
    elif "Stream" in args.data_path:
        idx_reward, features, labels = load_data_stream(
            args.data_path, args.train_size, args.block_num, args.block_size)
    ...
    p_size, p_step, p_center, p_bound = generate_parameter_space(
    ...
    for l in range(0, args.layer_num):
        drl = DrlDbscan(p_size, p_step[l], p_center, p_bound, args.device,
                        args.batch_size, args.step_num, features[0].shape[1])
        agents.append(drl)
    ...
    for b in range(0, args.block_num):
        ...
        std = open(args.log_path + '/Block' + str(b) + time_log + '/std.log',
                   'a')
        sys.stdout = std
        sys.stderr = std
        ...
        for l in range(0, args.layer_num):
            ...
            # testing
            cur_labels, cur_cluster_num, p_log = agent.detect(
        ...
        with open(args.log_path + '/Block' + str(b) + '/0_test.txt', 'a') as f:
            f.write(
                str(d_nmi) + "," + str(d_ami) + "," + str(d_ari) + "," +
                str(final_reward_test[1]) + "," + str(cur_cluster_num) + "," +
                str(len(label_dic_test)) + '\n')
        ...
        for l in range(0, args.layer_num):
            ...
            for i in range(1, args.episode_num):
                ...
                print("The size of Label Hash is: {0}".format(len(label_dic)),
                      flush=True)
                p_logs = np.array([[], []])
                nmi_logs = np.array([])
                ...
                print("Training the {0}-th layer agent......".format(l),
                      flush=True)
                cur_labels, cur_cluster_num, p_log, nmi_log, max_reward = agent.train(
                    i, idx_reward[b], features[b], labels[b], label_dic,
                    args.reward_factor)
                ...
                d_nmi, d_ami, d_ari = dbscan_metrics(labels[b], cur_labels)
                with open(
                        args.log_path + '/Block' + str(b) + time_log +
                        '/init_log.txt', 'a') as f:
                    f.write('episode=' + str(i) + ', layer=' + str(l) +
                            ',K-Means NMI=' + str(k_nmi) + '\n')
                    f.write(str(p_logs) + '\n')
        ...
        print("[ The best parameter is {0} ]".format(max_max_reward[1]),
              flush=True)
        print("[ The best parameter appears at {0} ]".format(first_meet_num),
              flush=True)
        cur_labels = label_dic[str(max_max_reward[1][0]) + str("+") +
                               str(max_max_reward[1][1])]
        cur_cluster_num = len(set(list(cur_labels)))
        print("[ The number of clusters is {0} ]".format(cur_cluster_num),
              flush=True)
        ...
        with open(args.log_path + '/Block' + str(b) + '/5_min_samples.txt',
                  'a') as f:
            f.write(str(max_max_reward[1][1]) + '\n')
        with open(args.log_path + '/Block' + str(b) + '/6_cur_cluster_num.txt',
                  'a') as f:
            f.write(str(cur_cluster_num) + '\n')
        with open(args.log_path + '/Block' + str(b) + '/7_first_num.txt',
                  'a') as f:
            f.write(str(first_meet_num) + '\n')
        with open(args.log_path + '/Block' + str(b) + '/8_all_num.txt',
                  'a') as f:
        ...
        for cur_labels in label_dic.values():
            reward_nmi = metrics.normalized_mutual_info_score(
                labels[b][idx_reward[b]], cur_labels[idx_reward[b]])
        ...
        with open(args.log_path + '/Block' + str(b) + '/max_nmi_logs.txt',
                  'a') as f:
